...
export function activate(context: vscode.ExtensionContext) {
    ...
    const openOutputCommand = vscode.commands.registerCommand('extension.openOutputChannel', () => {
        outputChannel.show();
        clearUnreadStatus();
    });
...
function connectToServer(serverUrl: string): void {
    try {
        ...
        socket.on('chat message', (data: { text: string; source: 'mobile' | 'vscode' }) => {
        ...
        const timestamp = getCurrentTimestamp();
        outputChannel.appendLine(`[Error - ${timestamp}] Failed to initialize: ${error}`);
    }
}
